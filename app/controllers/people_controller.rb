class PeopleController < ApplicationController
  ...
  def index
    #@people = Person.all
    if params[:type_person]
      @people=Person.where(:type_person=>params[:type_person])
    end
  ...
  def destroy
    @person = Person.find(params[:id])
    params[:type_person]= @person.type_person
    @person.destroy
    redirect_to(type_people_path(params[:type_person]))
  end
